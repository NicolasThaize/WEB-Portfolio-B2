...
class ArticlesListReplies extends React.Component{
  ...
  deleteReply = async (reply) => {
    ...
    await CommentsService.deleteReply(this.props.comment, reply).then(() => {
  ...
  render() {
    const { text, comment, userId, deleteValidation, wantedToDeleteReply, error } = this.state;
    return (
      <div>
        ...
        { comment.replies.map(reply => (
          <div key={reply.id} style={{marginLeft: "10px"}}>
            ...
            {reply.reply_author.id === userId ?
              <button onClick={() => this.wantToDelete(reply)}>Supprimer</button>
              : undefined }
            ...
            { error && wantedToDeleteReply.id === reply.id ? <p>{error}</p> : undefined }
