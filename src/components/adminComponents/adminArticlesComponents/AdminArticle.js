...
class AdminArticle extends React.Component {
  ...
  async componentDidMount() {
    this.toggleLoading();
    await ArticlesModule.getAllArticles()
      .then((r) => {
  ...
  apiModify = async (article) => {
    ...
    this.toggleLoading();
    await ArticlesModule.updateArticle(article.id, article)
      .then(() => {
        this.toggleLoading();
        this.triggerModify();
        this.getAllArticles();
  ...
  apiCreate = async (article) => {
    ...
    article.categories = categories;
    this.toggleLoading();
    await ArticlesModule.createArticle(article)
      .then(() => {
        this.toggleLoading();
        this.triggerCreate();
        this.getAllArticles();
  ...
  deleteFunc = (article) => {
    this.toggleLoading();
    ArticlesModule.deleteArticle(article)
      .then(() => {
        this.toggleLoading();
        this.getAllArticles();
      })
  ...
  render() {
    ...
    return (
      <div>
        ...
        <button type="button" onClick={this.triggerCreate}>
          {text.buttons.create}
        </button>
        {articles.length > 0 ? (
          <Pagination
